...
class course_lastenrolment implements rule_interface
{
    use score_limit;
    /**
     * @param int $param1
     * @param int $param2
     */
    function __construct(int $course_lastenrolment, int $time_unit)
    {
        $this->description  = 'Course Last Enrolment > 0';
        $this->state        = false;
        $this->score        = 0;

        $components = [
            'course_lastenrolment' => $course_lastenrolment
        ];
        $settings = [
            'time_unit' => $time_unit
        ];
    ...
    {
        return ($components['course_lastenrolment'] > 0);
    }
    function calculate_score(array $components, array $settings = []): float
    {
        return ((time() - $components['course_lastenrolment']) / $settings['time_unit']);
    }
}
