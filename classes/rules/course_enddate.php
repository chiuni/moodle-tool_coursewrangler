...
class course_enddate implements rule_interface
{
    use score_limit;
    function __construct(int $course_enddate, int $time_unit)
    {
        $this->description  = 'Course End Date != 0';
        $this->state        = false;
        $this->score        = 0;
    ...
    {
        return ($components['course_enddate'] != 0);
    }
    function calculate_score(array $components, array $settings = []): float
    {
        return ((time() - $components['course_enddate']) / $settings['time_unit']);
    }
}
